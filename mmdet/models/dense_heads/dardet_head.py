...
class DARDetHead(ATSSHead, FCOSHead):
    ...
    def forward_single(self, x, scale, scale_refine, stride, reg_denom):
        ...
        if self.bbox_norm_type == 'reg_denom':
            ...
            bbox_pred = scale(self.vfnet_reg(reg_feat_init)).float() #[:, 0:2, :, :]
            bbox_pred[:, 0:2, :, :] *= reg_denom
            bbox_pred[:, 2:4, :, :] = bbox_pred[:, 2:4, :, :].exp() * reg_denom
    ...
    def r_dcn_offset(self, bbox_pred, gradient_mul, stride):
        ...
        bbox_pred_grad_mul_offset[:, 16, :, :] = y3  # 9
        bbox_pred_grad_mul_offset[:, 17, :, :] = x3  # 9
    ...
    def _get_bboxes_single(self,
        ...
        if rescale:
            ...
            mlvl_rboxes[...,0:4] /= mlvl_rboxes.new_tensor(scale_factor)
