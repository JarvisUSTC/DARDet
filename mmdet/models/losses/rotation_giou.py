...
# shapely package implementation for giou
def polygon_box_iou(boxes1, boxes2):
    """
    polygon giou
    Args:
        boxes1: list((x1, y1, x2, y2, x3, y3, x4, y4))
    ...
    """
    iou_matrix = torch.zeros((len(boxes1),len(boxes2)),dtype=torch.float32).to(boxes1.device)
    for i,box1 in enumerate(boxes1):
        for j,box2 in enumerate(boxes2):
            Polygon1 = Polygon(box1.view(4,-1)).convex_hull
            Polygon2 = Polygon(box2.view(4,-1)).convex_hull
...
if __name__ == '__main__':
    r2 = (10, 10, 20, 20, -30)
    r1 = (10, 10, 10, 10, 30)
    Box1 = Variable(torch.tensor([10, 10, 20, 20, 10],dtype=float, device='cuda:0'),requires_grad=True)
    Box2 = Variable(torch.tensor([10, 10, 13, 10, 0],dtype=float, device='cuda:0'),requires_grad=True)
    boxes1 = []
    boxes2 = []
    ...
    print("using shapely package:",polygon_box_iou(torch.tensor([boxes1]),torch.tensor([boxes2])))
    # print(intersection_area(r1, r2))
    print("using my iou implementation",IoU_Rotated_Rectangle(Box1,Box2))
    IoU_Rotated_Rectangle(Box1,Box2).backward()
    print("the grad of two rectangle after backward:",Box1.grad,Box2.grad)
    temp_box1 = Variable(torch.tensor([10, 10, 20, 20, 10*pi/180],dtype=torch.float32, device='cuda:0'),requires_grad=True)
    temp_box2 = Variable(torch.tensor([10, 10, 13, 10, 0],dtype=torch.float32, device='cuda:0'),requires_grad=True)
    print("using piou implementation",box_iou_rotated_differentiable(temp_box1.unsqueeze(0),temp_box2.unsqueeze(0)))
    box_iou_rotated_differentiable(temp_box1.unsqueeze(0),temp_box2.unsqueeze(0)).backward()
    print("the grad of two rectangle after backward:",temp_box1.grad,temp_box2.grad)
    
    temp_target = torch.tensor([10,10,20,10,45*pi/180],dtype=torch.float32, device='cuda:0')
    temp_target_my = torch.tensor([10,10,20,10,45*pi/180],dtype=torch.float32, device='cuda:0')
    iou_temp = []
    iou_my = []
    for angle in range(0,360):
        angle = angle/2
        # temp_target = torch.tensor([10,10,10,10,angle*pi/180],dtype=torch.float32, device='cuda:0')
        # temp_target_my = torch.tensor([10,10,10,10,angle*pi/180],dtype=torch.float32, device='cuda:0')
        temp_box_my = torch.tensor([10, 10, 20, 10, angle*pi/180],dtype=torch.float32, device='cuda:0')
        temp_box = torch.tensor([10, 10, 20, 10, angle*pi/180],dtype=torch.float32, device='cuda:0')
        iou_temp.append(box_iou_rotated_differentiable(temp_box.unsqueeze(0),temp_target.unsqueeze(0)).cpu())
        iou_my.append(IoU_Rotated_Rectangle(temp_box_my,temp_target_my, True).cpu())
    plt.figure()
    plt.plot(np.array(range(0,360))/2, iou_temp, marker='o', color='blue', label='piou')
    plt.plot(np.array(range(0,360))/2, iou_my, marker='*', color='orange', label='iou')
    plt.legend()
