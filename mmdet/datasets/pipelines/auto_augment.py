...
class Rotate:
    ...
    def __init__(self,
        ...
        # clockwise rotation.
#level_to_value(level, max_rotate_angle)
        self.center = center
        self.img_fill_val = img_fill_val
        self.seg_ignore_label = seg_ignore_label
        self.prob = prob
    ...
    def _rotate_bboxes_masks(self,  results,   angle,   center=None,    scale=1.0,    fill_val=0):
        h, w, c = results['img_shape']
        masks = results['gt_masks']
        labels=results['gt_labels'] 
        boxes= results['gt_bboxes']
        img=results['img']
        gt_masks=np.array(masks.masks).squeeze().reshape(-1,8)
        ...
        rotated_img = cv2.warpAffine(img, matrix, (w, h), borderValue=self.img_fill_val)
        
        if gt_masks.size<8:
            return
        x1, y1, x2, y2,x3,y3,x4,y4 = np.split(  gt_masks, gt_masks.shape[-1], axis=-1)
        coordinates = np.stack([[x1, y1], [x2, y2],     [x3, y3],      [x4, y4]])  # [4, 2, nb_bbox, 1]
        # pad 1 to convert from format [x, y] to homogeneous
        # coordinates format [x, y, 1]
        coordinates = np.concatenate( (coordinates,  np.ones((4, 1, coordinates.shape[2], 1), coordinates.dtype)),  axis=1)  # [4, 3, nb_bbox, 1]
        ...
        all_img_box=((w/2,h/2),(w,h),0)
        # #show
        # img=results['img']
        # pts_all=np.array(cv2.boxPoints(all_img_box)).astype(np.int0).reshape((-1,2))
        # cv2.polylines(img,[pts_all],True,(255,255,255))
        assert(rotated_coords.shape[0]==labels.shape[0])
        r_points,mask_points=[],[]
        gt_labels=[]
        for i in range(rotated_coords.shape[0]):
            rect=cv2.minAreaRect(rotated_coords[i].astype(np.float32))
            if not self.auto_bound:
                int_pts = cv2.rotatedRectangleIntersection(rect, all_img_box)[1]#求两个旋转矩形的交集，并返回相交的点集合
                if int_pts is None : #is None and (rect[0][0]<0 or rect[0][1]<0  or rect[0][0]>w or rect[0][1]>h):
                    # print(rect)
                    continue
                else:
                    order_pts = cv2.convexHull(np.array(int_pts), returnPoints=True)#求点集的凸边形
                    #show
                    pts_all=np.array(order_pts).reshape((-1,2))
                    # cv2.polylines(img,[pts_all],True,(0,255,0))

                    int_area = cv2.contourArea(order_pts)#计算当前点集合组成的凸边形的面积
                    if int_area<rect[1][0]*rect[1][1]*self.out_range_ratio or int_area< self.min_area:
                        continue
                    rect2=cv2.minAreaRect(pts_all.astype(np.float32))
                    rect= rect2
            points=cv2.boxPoints(rect).reshape(-1,8)
        ...
        r_points=np.array(r_points).squeeze().reshape(-1,8)
        min_x, min_y = np.min(  r_points[:, 0::2], axis=1), np.min( r_points[:, 1::2], axis=1)
        max_x, max_y = np.max(r_points[:, 0::2], axis=1), np.max( r_points[:, 1::2], axis=1)
        ...
        results['gt_bboxes'] = np.stack([min_x, min_y, max_x, max_y], axis=-1).astype(boxes.dtype)
        results['gt_labels'] =np.array(gt_labels)#.squeeze()
        results['gt_masks']  = PolygonMasks(  mask_points, h,  w)
    ...
    def __call__(self, results):
        ...
        self._rotate_bboxes_masks(results, angle, center, self.scale, fill_val=0)
        self._rotate_seg(
