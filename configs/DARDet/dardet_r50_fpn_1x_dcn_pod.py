...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(
            ...
            iou_weighted=True,
            loss_weight=1.0),
            # loss_rbox = dict(type='ModulatedRotationLoss', loss_weight=1.5),
            loss_rbox=dict( type='RotatedIoULoss', loss_weight=1.5, iou='piou'),
            loss_rbox_refine=dict( type='RotatedIoULoss', loss_weight=2.0, iou='piou')),
...
train_pipeline = [
    ...
    dict(type='Rotate', prob=0.9, auto_bound=True, max_rotate_angle=360, img_fill_val=0),
    dict(
        type='AutoAugment',
        policies=[[
            dict(
...
data = dict(
    ...
    train=dict(
        classes=classes,
        type=dataset_type,
        ann_file=[data_root + 'annotations/train_2.json'],# data_root + 'annotations/train_1.json', data_root + 'annotations/train_3.json', data_root + 'annotations/train_4.json'],
...
resume_from = None#'/datadisk/v-jiaweiwang/Checkpoints/MMDetection/DARDet/exp1_v11/latest.pth'
